...
//所有批次的信息的查看
CheckBatch::CheckBatch(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CheckBatch)
{
    ui->setupUi(this);
...
void CheckBatch::on_refresh_batch_clicked()
{
    batchList = dbhelper::getInstance()->QgetcheckBatch();
    ...
    if( l == 0)
    {
        ...
        QTableWidgetItem *item = new QTableWidgetItem("还没有批次在审核哦~~~");
...
void CheckBatch::on_tableWidget_clicked(const QModelIndex &index)
{
    ui->treeWidget->clear();
    //树顶:批次号
