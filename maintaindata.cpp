...
{
    QList<Product> proList = dbhelper::getInstance()->QgetData();
    load(proList);
}
...
void MaintainData::on_data_delete_clicked()
{
    ...
    {
        case QMessageBox::Ok:
            qres = dbhelper::getInstance()->QdeleteData(userId,curproduct);
            if(!qres.success)
            {
...
void MaintainData::on_data_deleteAll_clicked()
{
    ...
    {
        case QMessageBox::Ok:
            qres = dbhelper::getInstance()->QdeleteAllData(userId);
            if(!qres.success)
            {
                QMessageBox::warning(this,tr("warnig"),qres.msg);
                return;
...
void MaintainData::onHeaderClicked(int col)
{
    ...
    QList<Product> proList = dbhelper::getInstance()->QgetDataorderby(col,pattern);//0:batchid 1:tray 2:number 3:type 4:time 5:flag      pattern: 1:increase 2: decrease
    load(proList);
}
