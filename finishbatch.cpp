...
//所有批次的信息的查看
FinishBatch::FinishBatch(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FinishBatch)
{
    ui->setupUi(this);
...
void FinishBatch::on_refresh_batch_clicked()
{
    batchList = dbhelper::getInstance()->QgetfinishBatch();
    ...
    if( l == 0)
    {
        ...
        QTableWidgetItem *item = new QTableWidgetItem("还没有已完成的批次哦~~~");
...
void FinishBatch::on_tableWidget_clicked(const QModelIndex &index)
{
    ui->treeWidget->clear();
    //树顶:批次号
